  ...
  return (
    ...
    <div className="w-full h-[50vh] sm:h-full md:h-[600px] sticky z-10 top-0 md:relative">
      ...
      <div className="absolute bottom-80 md:left-10 lg:left-[4.2rem] hidden md:flex items-center">
        <img src={discoverOne} alt="Logo" className="h-5 mr-2 opacity-80 hover:opacity-100 transition-opacity duration-300" />
      </div>
    ...
    <div className="w-[90%] max-w-[1720px] h-auto p-4 md:p-16 bg-[#f8f8f8] shadow-lg rounded-lg mx-auto mt-[-2rem] md:mt-[-6rem] relative z-10">
      <div className="flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <div className="md:w-auto w-full flex justify-end">
          <div className="cursor-pointer hover:scale-110 transition-transform duration-300" onClick={toggleModal}>
            <FilterSvg />
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mt-4 items-end">
        ...
        <div className="flex flex-col">
          ...
          <div className="flex items-center justify-around bg-gray-100 rounded-md p-2 h-12 hover:shadow-md transition-shadow duration-300">
            <button onClick={decrementGuests} className="bg-med-green w-7 h-7 flex items-center justify-center text-white rounded-full hover:scale-110 transition-transform duration-300">-</button>
            <p className="font-medium text-[#717A7C] text-lg">{formState.guests} guest{formState.guests > 1 ? "s" : ""}</p>
            <button onClick={incrementGuests} className="bg-med-green w-7 h-7 flex items-center justify-center text-white rounded-full hover:scale-110 transition-transform duration-300">+</button>
          </div>
        </div>
        <div className="flex justify-center items-center md:col-span-1">
          <button onClick={handleSearch} className="w-full bg-med-green py-2 rounded-md text-white font-medium text-xl hover:scale-95 hover:shadow-md transition-colors duration-300">
            Search Packages
          </button>
        </div>
      </div>
    ...
    <FilterBox showModal={formState.showModal} setShowModal={toggleModal} onClose={handleClose} />
  </section>
