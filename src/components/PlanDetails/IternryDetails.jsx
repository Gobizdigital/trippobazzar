...
function IternryDetails({ data }) {
  ...
  const { searchData, setSearchData, selectedPricing, setSelectedPricing, setSelectedPricePerPerson } = useSearch()
  ...
  const handleHotelArray = (hotel, hdetail) => {
    const isHotelSelected = selectedHotel.some((item) => item._id === hdetail._id)

    const isLocationSelected = selectedHotel.some((item) => item.location === hotel.location)

    if (isHotelSelected) {
      setSelectedHotel((prevSelected) => prevSelected.filter((item) => item._id !== hdetail._id))
    } else {
  ...
  const calculateDiscountedPrice = () => {
    ...
    let mainPrice = selectedPricing ? selectedPricing * searchData.guests : data?.price * searchData.guests

    // Add extra bed and CNB charges only if selectedPricing is chosen
    if (selectedPricing) {
      const selectedPackage = data?.pricing?.find((p) => p.basePrice === selectedPricing)

      const extraBedCharge = additionalServices?.extraBed ? selectedPackage?.extraBedCharge || 0 : 0
    ...
    const discount = Math.min((totalCost * selectedCoupon.discountPercentage) / 100, selectedCoupon.maxDiscount)
  ...
  const validateHotelSelection = () => {
    ...
    const missingLocations = allLocations.filter((location) => !selectedLocations.includes(location))

    if (missingLocations.length > 0) {
      setValidationError(`Please select a hotel for ${missingLocations.join(", ")}`)
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      ...
      if (isPricingPanelOpen && panel && !panel.contains(event.target) && !button.contains(event.target)) {
        setIsPricingPanelOpen(false)
      }
    }
  ...
  useEffect(() => {
    if (data && data.pricing && data.pricing.length > 0 && selectedPricing === null) {
  ...
  return (
    <div className="w-full md:w-[90%] mx-auto bg-white flex flex-col lg:flex-row font-poppins">
      ...
      <div className="w-full lg:w-4/5 p-4 ">
        ...
        <nav className="flex w-full gap-4 em:gap-10 border-[.5px] px-4 em:px-0 scrollbar-hide overflow-x-auto rounded-lg justify-start em:justify-center py-4 whitespace-nowrap mb-4 text-center text-sm sm:text-base">
          {["Plan Details", "Hotel", "Policies"].map((tab, tabIdx) => (
            <li
              key={tabIdx}
              className={`cursor-pointer list-none ${activeTab === tab ? "text-green-500 font-semibold" : ""}`}
        ...
        {/* Dynamic Content */}
        <div className={fadeState === "fadeIn" ? "animate-fadeIn" : "animate-fadeOut"}>
          {activeTab === "Plan Details" && (
            <div className="space-y-6">
              {data?.dayDescription?.map((day, dayIdx) => (
                <div className="border-[.5px] px-4 py-4 rounded-lg" key={dayIdx}>
                  <h3 className="text-base md:text-lg leading-6 md:leading-7 font-bold">{day.dayTitle}</h3>
                  ...
                  <span className="sm:text-base text-sm" dangerouslySetInnerHTML={{ __html: day?.dayDetails }} />
                </div>
              ))}
            </div>
          )}
            ...
            <div className="border p-6 rounded-3xl relative bg-white text-sm sm:text-base shadow-sm">
              ...
              {data?.pricing && (
                <div className="mb-3 relative">
                  <label className="block text-gray-700 font-semibold mb-2">Select Pricing</label>
                  <button
                    id="pricing-button"
                  ...
                  >
                    <span className="text-gray-800 truncate">{getSelectedPricingLabel()}</span>
                    <ChevronDown
                  ...
                  {isPricingPanelOpen && (
                    ...
                    >
                      {getUniqueGuestCounts().map((guestCount) => (
                        <div key={guestCount} className="border-b last:border-b-0">
                          ...
                          {getFilteredOptions(guestCount).map((option) => (
                            <div
                              ...
                              className={`px-4 py-3 cursor-pointer hover:bg-gray-50 transition-colors flex items-center justify-between ${
                                selectedPackageId === option.id ? "bg-green-50" : ""
                              }`}
                            >
                              <div className="flex-1">
                                <div className="flex justify-between items-center">
                                  <span
                                    className={`${
                                      selectedPackageId === option.id ? "font-medium text-green-600" : "font-medium"
                                    }`}
                                  >
                                    {option.packageType}
                                  </span>
                                  <span className="text-green-600 font-semibold">₹{option.value}</span>
                                </div>
              ...
              {selectedPricing && (
                <div className="mt-4 space-y-2">
                  {data?.pricing?.find((p) => p.basePrice === selectedPricing)?.extraBedCharge > 0 && (
                    <label className="flex items-center gap-2 cursor-pointer hover:bg-gray-50 p-2 rounded-md transition-colors">
                      ...
                      <span className="flex-1">
                        Extra Bed (+ ₹{data?.pricing?.find((p) => p.basePrice === selectedPricing)?.extraBedCharge || 0}
                        )
                      </span>
                      {additionalServices?.extraBed && <IoCheckmarkCircle className="text-green-500 text-lg" />}
                    </label>
                  )}

                  {data?.pricing?.find((p) => p.basePrice === selectedPricing)?.CNB > 0 && (
                    <label className="flex items-center gap-2 cursor-pointer hover:bg-gray-50 p-2 rounded-md transition-colors">
                      <input
                        type="checkbox"
                      ...
                      {additionalServices?.cnb && <IoCheckmarkCircle className="text-green-500 text-lg" />}
                    </label>
                  )}

                  {data?.pricing?.find((p) => p.basePrice === selectedPricing)?.CWB > 0 && (
                    <label className="flex items-center gap-2 cursor-pointer hover:bg-gray-50 p-2 rounded-md transition-colors">
                      <input
                        type="checkbox"
                      ...
                      />
                      <span className="flex-1">
                        Child With Bed (CWB) (+ ₹{data?.pricing?.find((p) => p.basePrice === selectedPricing)?.CWB || 0}
                        )
                      </span>
                      {additionalServices?.cwb && <IoCheckmarkCircle className="text-green-500 text-lg" />}
                    </label>
                  )}
                </div>
              )}
              ...
              <div className="mt-6 p-3 bg-gray-50 rounded-lg">
                ...
                <p className="border rounded-xl mb-3 text-blue-500 flex justify-center items-center gap-4 py-1 text-xs mt-3 bg-white">
                  <span className="text-2xl">🎉</span> No Additional or Hidden Costs!!!
                </p>

                <div className="border-b my-3 border-gray-200 w-full"></div>

                <p className="font-semibold gap-2 flex items-start justify-between">
                  <span>Total Payable:</span> <span className="text-green-600">₹ {calculateDiscountedPrice()}</span>
                </p>
              </div>

              <button
                onClick={handleSubmit}
                className="bg-green-600 text-lg cursor-pointer text-white inline-flex py-3 px-4 rounded-xl w-full justify-center mt-4 hover:bg-green-700 transition-colors font-medium"
              >
                Confirm and Book
              </button>

              {/* Hotel selection reminder */}
              {data?.hotels && data.hotels.length > 0 && selectedHotel.length < data.hotels.length && (
                ...
                userDetails.Coupons.map((item) => {
                  return (
                    <div
                      ...
                      ${selectedCoupon.id === item._id ? "border-green-600" : "border-white"} 
                      ${
                        selectedCoupon.id !== null && selectedCoupon.id !== item._id
                          ? "hover:border-[#e5e7eb]"
                    ...
                    >
                      ...
                      <div className="w-full">
                        ...
                        <p className="text-xs mb-3 text-gray-500">{item.couponDescription}</p>

                        <div className="flex justify-between">
                          <p className={`text-green-500 text-sm rounded`}>
                            {selectedCoupon.id === item._id ? "Applied!" : "Apply"}
                          </p>
      ...
      {/* Image Modal */}
      {imgmodal.isOpen && <CarosalImageModal images={imgmodal.img} handleCloseModal={closeModal} />}
    </div>
  )
}
