...
export default function HotelsPlan({ handleHotelArray, selectedHotel, data, openModal, selectedPackageType }) {
  return (
    <div className="space-y-6">
      {data?.hotels?.map((hotel) => {
        ...
        const filteredHotelDetails = selectedPackageType
          ? hotel?.hotelDetails.filter((hdetail) => hdetail.hotelType === selectedPackageType)
          : hotel?.hotelDetails

        return (
          <div key={hotel._id || hotel.location} className="pb-6 last:pb-0">
            <div className="flex items-center justify-between mb-3">
              <h4 className="font-medium text-lg text-gray-800 border-l-4 border-green-500 pl-3">{hotel.location}</h4>
                ...
                <div className="flex flex-row">
                  ...
                  <div className="p-3 w-2/3">
                    <div className="flex justify-between items-start">
                      <h3 className="font-medium text-base text-gray-800">{selectedHotelForLocation.hotelName}</h3>
                      ...
                      <div className="flex">
                        {[...Array(5)].map((_, i) => {
                          if (i < Math.floor(selectedHotelForLocation.hotelRating)) {
                            return <MdStar key={i} className="text-yellow-400 text-sm" />
                          } else if (i < selectedHotelForLocation.hotelRating) {
                            return <MdStarHalf key={i} className="text-yellow-400 text-sm" />
                          } else {
                            return <MdStarOutline key={i} className="text-gray-300 text-sm" />
                          }
                        })}
                      </div>
                ...
                {filteredHotelDetails.length > 0 ? (
                  filteredHotelDetails.map((hdetail, hdetailIndex) => (
                    ...
                    >
                      ...
                      <div className="p-3">
                        <h3 className="font-medium text-sm text-gray-800 truncate">{hdetail.hotelName}</h3>

                        <div className="mt-2 flex justify-between items-center">
                          <div className="flex">
                            {[...Array(5)].map((_, i) => {
                              if (i < Math.floor(hdetail.hotelRating)) {
                                return <MdStar key={i} className="text-yellow-400 text-xs" />
                              } else if (i < hdetail.hotelRating) {
                                return <MdStarHalf key={i} className="text-yellow-400 text-xs" />
                              } else {
                                return <MdStarOutline key={i} className="text-gray-300 text-xs" />
                              }
                            })}
                          </div>
                  ...
                  <div className="col-span-full py-8 text-center text-gray-500">
                    No hotels available for the selected package type. Please select a different package.
                  </div>
                )}
              </div>
            )}
