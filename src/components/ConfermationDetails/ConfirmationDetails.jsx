...
const GuestForm = () => {
  ...
  const validateForm = () => {
    ...
    guestDetails.guests.forEach((guest, index) => {
      ...
      if (guest.passportNumber && !guest.issuingCountry) {
        newErrors[`guest_${index}_issuingCountry`] =
          "Issuing country is required if passport number is provided";
        isValid = false;
      }

      if (guest.passportNumber && !guest.passportExpiry) {
        newErrors[`guest_${index}_passportExpiry`] =
          "Passport expiry date is required if passport number is provided";
        isValid = false;
    ...
    // Validate GST details if any GST field is filled
    if (
      guestDetails.gst.gstNumber ||
      guestDetails.gst.state ||
      guestDetails.gst.address
    ) {
      if (!guestDetails.gst.state) {
        newErrors["gst.state"] = "State is required for GST";
        isValid = false;
      }

      if (!guestDetails.gst.gstNumber) {
        newErrors["gst.gstNumber"] = "GST number is required";
        isValid = false;
      } else if (
      ...
      if (!guestDetails.gst.address) {
        newErrors["gst.address"] = "Address is required for GST";
        isValid = false;
      }
    }
